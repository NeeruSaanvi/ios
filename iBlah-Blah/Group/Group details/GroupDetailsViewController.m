...
@interface GroupDetailsViewController ()<UIScrollViewDelegate,UIGestureRecognizerDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate>{
...
- (void)viewDidLoad {
    ...
    [appDelegate().socket emit:@"showAllGroupImages" with:@[USERID,[_dictGroup objectForKey:@"id"]]];
    
    [appDelegate().socket emit:@"getAllEventsGroups" with:@[USERID,[_dictGroup objectForKey:@"id"],@"0"]];
    
    [appDelegate().socket emit:@"getAllPostGroups" with:@[USERID,[_dictGroup objectForKey:@"id"],currentAllPostPage]];
    
     [appDelegate().socket emit:@"showGroupDetail" with:@[USERID,[_dictGroup objectForKey:@"id"]]];
...
- (void)receivedNotification:(NSNotification *) notification {
    ...
    if ([[notification name] isEqualToString:@"GroupDetails"]) {
        [ind removeFromSuperview];
        ...
        NSDictionary *dict=[arryGroupInforamtion objectAtIndex:0];
        NSString *strAdmin=[dict objectForKey:@"admin"];
        
        NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
        NSString *USERID = [prefs stringForKey:@"USERID"];
        if([strAdmin isEqualToString:USERID]){
            NSMutableArray *arrRightBarItems = [[NSMutableArray alloc] init];
            UIButton *btnClear = [UIButton buttonWithType:UIButtonTypeCustom];
            [btnClear setTitle:@"Delete" forState:UIControlStateNormal];
            btnClear.frame = CGRectMake(0, 0, 32, 32);
            [btnClear addTarget:self action:@selector(cmdDelete:) forControlEvents:UIControlEventTouchUpInside];
            
            UIButton *btnClear1 = [UIButton buttonWithType:UIButtonTypeCustom];
            [btnClear1 setTitle:@"Edit" forState:UIControlStateNormal];
            btnClear1.frame = CGRectMake(0, 0, 32, 32);
            [btnClear1 addTarget:self action:@selector(cmdEditGroup:) forControlEvents:UIControlEventTouchUpInside];
            
            UIBarButtonItem *btnSearchBar = [[UIBarButtonItem alloc] initWithCustomView:btnClear];
            UIBarButtonItem *btnSearchBar1 = [[UIBarButtonItem alloc] initWithCustomView:btnClear1];
           ...
           joinedUser=NO;
            ...
            btnClear.frame = CGRectMake(0, 0, 32, 32);
            if(joinedUser){
                [btnClear setTitle:@"Exit" forState:UIControlStateNormal];
                [btnClear addTarget:self action:@selector(cmdExitGroup:) forControlEvents:UIControlEventTouchUpInside];
            }else{
                [btnClear setTitle:@"Join" forState:UIControlStateNormal];
                [btnClear addTarget:self action:@selector(cmdJoin:) forControlEvents:UIControlEventTouchUpInside];
            }
...
//        [_tblMyGroup reloadData];
    ...
    }else  if ([[notification name] isEqualToString:@"refreshGroupDetails"]) {
        NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
        NSString *USERID = [prefs stringForKey:@"USERID"];
        [appDelegate().socket emit:@"showAllGroupImages" with:@[USERID,[_dictGroup objectForKey:@"id"]]];
        ...
        [appDelegate().socket emit:@"showGroupDetail" with:@[USERID,[_dictGroup objectForKey:@"id"]]];
    }else  if ([[notification name] isEqualToString:@"UpdatePost"]) {
        NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
        NSString *USERID = [prefs stringForKey:@"USERID"];
          [appDelegate().socket emit:@"getAllPostGroups" with:@[USERID,[_dictGroup objectForKey:@"id"],currentAllPostPage]];
    }
...
-(void)cmdExitGroup:(id)sender{
    [self.view addSubview:ind];
    joinedUser=NO;
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    NSString *USERID = [prefs stringForKey:@"USERID"];
    [appDelegate().socket emit:@"removeFromGroup" with:@[USERID,[_dictGroup objectForKey:@"id"]]];
    
    [self performSelector:@selector(refreshDetails) withObject:nil afterDelay:1.5];
}
-(void)cmdJoin:(id)sender{
    [self.view addSubview:ind];
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    NSString *USERID = [prefs stringForKey:@"USERID"];
    [appDelegate().socket emit:@"addToGroup" with:@[USERID,[_dictGroup objectForKey:@"id"]]];
...
-(void)cmdDelete:(id)sender{
  //  emit("deleteGroup", user_id, group_id)
    [self.view addSubview:ind];
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    NSString *USERID = [prefs stringForKey:@"USERID"];
    [appDelegate().socket emit:@"deleteGroup" with:@[USERID,[_dictGroup objectForKey:@"id"]]];
   
    [AlertView showAlertWithMessage:@"Your group deleted successfully" view:self];
...
- (CGFloat) tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section
{
    ...
    else if(tableView==_tblInformation){
        if(section==0){
            return 0;
        }else{
            return 50;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if(_tblInformation==tableView){
        ...
        return hedderView;
    }else if(_tblActivity==tableView){
        if(!joinedUser){
            return nil;
        }
    UIView *hedderView=[[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_SIZE.width, 50)];
    ...
    return hedderView;
        ...
        UIButton *btnEdit = [UIButton buttonWithType:UIButtonTypeSystem];
        [btnEdit addTarget:self
                    action:@selector(cmdAddEvent:)
          forControlEvents:UIControlEventTouchUpInside];//camera editPost
        [btnEdit setTitle:@"Add Event" forState:UIControlStateNormal];
        btnEdit.frame = CGRectMake(10, 5, SCREEN_SIZE.width-20, 40);
        btnEdit.tintColor = [UIColor colorWithRed:0/255.0 green:160/255.0 blue:223/255.0 alpha:1.0];
        [hedderView addSubview:btnEdit];
...
- (void)smallButtonTapped1:(UITapGestureRecognizer *)tapRecognizer {
...
-(void)changeButtonColor{
    [_btnInformation setBackgroundColor:[UIColor clearColor]];
    ...
    [_btnEvents setBackgroundColor:[UIColor clearColor]];
...
-(UIView *)tblInfoSection1:(NSDictionary *)dict{
    ...
    strUrl=[NSString stringWithFormat:@"%sthumb/%@",BASEURl,[dict objectForKey:@"group_image"]];
...
-(UIView *)tblInfoSection2:(NSDictionary *)dict{
    
    UIView *viewDesign=[[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_SIZE.width, 80)];
    viewDesign.backgroundColor=[UIColor clearColor];
    UIScrollView *scrollView=[[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_SIZE.width, 90)];
    ...
    for (int i=0; i<arryGroupMember.count; i++) {
        ...
        if([strIsAdmin isEqualToString:@"1"]){
            UILabel *lblIsAdmin = [[UILabel alloc] initWithFrame:CGRectMake(xAxis, 10,50,10)];
        ...
        UILabel *lblUserName = [[UILabel alloc] initWithFrame:CGRectMake(xAxis,70,width,20)];
        ...
        AsyncImageView *imgUser=[[AsyncImageView alloc]initWithFrame:CGRectMake(xAxis+(width/2)-25, 20,50,50)];
...
-(UIView *)tblActivity:(NSDictionary *)dict indexPath:(NSIndexPath *)indexPath{
    ...
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(smallButtonTapped1:)];
