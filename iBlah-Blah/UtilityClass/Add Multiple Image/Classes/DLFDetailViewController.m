...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *backButton = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Back", nil) style:UIBarButtonItemStyleDone target:self action:@selector(didTapNextButton:)];
...
- (void)toggleSelectedIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.selectionManager containsAsset:asset]) {
        ...
        if (indexPath) {
         selected =   [self.selectionManager addSelectedAsset:asset];
           // selected = YES;
        }
    }
