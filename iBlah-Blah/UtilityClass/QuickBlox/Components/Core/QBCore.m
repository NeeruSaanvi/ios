...
- (void)loginWithCurrentUser {
    ...
    [QBRequest logInWithUserLogin:self.currentUser.email
                     ...
                     successBlock:^(QBResponse * _Nonnull response, QBUUser * _Nullable user)
     {
         ...
         NSString *QBID =[NSString stringWithFormat:@"%lu",user.ID] ;
         [appDelegate().socket emit:@"update_caller_id" with:@[USERID,QBID]];
...
- (void)loginWithCurrentUser:(QBUUser *)user1 {
    ...
    __block dispatch_block_t connectToChat = ^{
        ...
        __block QBUUser *userLogin=user1;
        
        __typeof(weakSelf)strongSelf = weakSelf;
       
        
        userLogin.password = QB_DEFAULT_PASSOWORD;
        strongSelf.currentUser.fullName=userLogin.fullName;
        strongSelf.currentUser.email=userLogin.email;
        QBUUser *user = userLogin;
   ...
   // [self setLoginStatus:@"Login with current user ..."];
    ...
    [QBRequest logInWithUserEmail:user1.email password:QB_DEFAULT_PASSOWORD successBlock:^(QBResponse * _Nonnull response, QBUUser * _Nonnull tUser) {
        self.isAuthorized = YES;
         [self.profile synchronizeWithUserData:tUser];
        connectToChat();
        connectToChat = nil;
        NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
        NSString *deviceToken = [prefs stringForKey:@"deviceTokenStrKey"];
   ...
   // newUser.login = [NSUUID UUID].UUIDString;
    ...
    [QBRequest signUp:newUser
         successBlock:^(QBResponse * _Nonnull response, QBUUser * _Nullable user)
     {
         ...
         NSString *QBID =[NSString stringWithFormat:@"%lu",user.ID] ;
          [appDelegate().socket emit:@"update_caller_id" with:@[USERID,QBID]];
        // mSocket.emit("update_caller_id", user_id, quickblox_user_id);
