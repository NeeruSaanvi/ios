...
- (void)updateOfferInfo {
    ...
    NSString *UserID=[NSString stringWithFormat:@"%@",self.session.initiatorID];
    
    [QBRequest usersWithIDs:@[UserID] page:[QBGeneralResponsePage responsePageWithCurrentPage:1 perPage:10] successBlock:^(QBResponse *response, QBGeneralResponsePage *page, NSArray *users) {
        
        QBUUser *user=[users objectAtIndex:0];
         self.colorMarker.title = user.fullName;
