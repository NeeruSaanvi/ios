...
- (void)viewDidLoadForImageMode {
    ...
    if ([self.optionsDelegate respondsToSelector:@selector(imageViewerShouldFadeThumbnailsDuringPresentationAndDismissal:)]) {
        if ([self.optionsDelegate imageViewerShouldFadeThumbnailsDuringPresentationAndDismissal:self]) {
            ...
            _btnSaveImage =[[UIButton alloc]init];
            _btnSaveImage = [UIButton buttonWithType:UIButtonTypeSystem];
            _btnSaveImage.frame = CGRectMake(SCREEN_SIZE.width-50,SCREEN_SIZE.height-120,30,30);
            _btnSaveImage.titleLabel.font = [UIFont fontWithName:@"HelveticaNeue-Bold" size:15.0];
            [_btnSaveImage setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            [_btnSaveImage setBackgroundImage:[UIImage imageNamed:@"download"] forState:UIControlStateNormal];
            [_btnSaveImage addTarget:self  action:@selector(cmdSave:) forControlEvents:UIControlEventTouchUpInside];
            [self.view addSubview:_btnSaveImage];
...
-(void)cmdSave:(id)sender{
    ...
    [AlertView showAlertWithMessage:@"Image Saved" view:self];
}
